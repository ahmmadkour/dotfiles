;; Mostly copied from https://github.com/daviwil/emacs-from-scratch
...
(use-package lsp-mode
  ...
  (setq lsp-rust-analyzer-diagnostics-enable t)
  (setq lsp-rust-analyzer-diagnostics-experimental-enable t)
...
(defun warn-if-file-changed ()
  "Warns the user if the file on disk has changed while the buffer has unsaved edits."
  (when (and (buffer-file-name)   ;; Only check if buffer is associated with a file
             (buffer-modified-p)  ;; Buffer has unsaved changes
             (not (verify-visited-file-modtime (current-buffer))))  ;; File changed
    (let ((file-msg (propertize (buffer-file-name)
                                'face '(:foreground "yellow" :weight bold))))
      (message "File '%s' changed on disk while you have unsaved edits!" 
               file-msg))))

(add-hook 'auto-revert-mode-hook #'warn-if-file-changed)
(run-with-timer 2 2 #'warn-if-file-changed)  ;; Check every 2 seconds

(setq backup-directory-alist
      `(("." . ,(concat user-emacs-directory "~/.config/emacs/files-cache"))))
...
(unless (boundp 'compilation-environment)
