...
[[https://agentclientprotocol.com][Agent Client Protocol (ACP)]]. Rather than driving a CLI in a terminal, agent-shell launches
an ACP backend process and renders the session in a =comint= buffer. For Claude that backend
is =claude-agent-acp=, built on the Claude Agent SDK.
...
=SPC a a= (=agent-shell=) is the one to remember — it does what you mean: no shell in the
project yet, it offers to start one; already in a shell, it toggles the window; region
active, it carries the region over as input. The rest of the =SPC a= group is defined with
the [[*Keybinding][leader keys]].
