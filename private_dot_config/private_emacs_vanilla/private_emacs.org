...
=wgrep= makes grep result buffers editable. Combined with =counsel-rg= (=SPC /=), this enables project-wide find-and-replace: search → =C-c C-o= (ivy-occur) → =C-c C-p= (wgrep) → edit matches → =C-c C-c= apply to all files.
